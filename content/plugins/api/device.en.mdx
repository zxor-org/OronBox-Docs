...
Except for `connect`, methods accept an optional `deviceId` parameter; omit it to target the current active device. `connect` requires the device to connect to.
